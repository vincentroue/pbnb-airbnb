...
format:
  html:
    # Theme/toc/grid hérités du _quarto.yml (website pbnb) — pas d'override local
    code-fold: true
execute:
  cache: true
  warning: false
...
render_cluster_cards(hcpc, indiv_df, ddict = DDICT, actives = rownames(pca$var$coord), top_n_para = 6)
```
...
(plot_ind_cluster(pca, hcpc, axes = c(1, 2), label_parangons = TRUE) + coord_cartesian(xlim = .xl, ylim = .yl)) +
(plot_ind_cluster(pca, hcpc, axes = c(1, 3), label_parangons = TRUE) + coord_cartesian(xlim = .xl, ylim = .yl)) +
...
p12 <- plot_ind_cluster(pca, hcpc, axes = c(1, 2), label_parangons = TRUE) +
  coord_cartesian(xlim = shared_xlim, ylim = shared_ylim)
p13 <- plot_ind_cluster(pca, hcpc, axes = c(1, 3), label_parangons = TRUE) +
  coord_cartesian(xlim = shared_xlim, ylim = shared_ylim)
p12 + p13 +
