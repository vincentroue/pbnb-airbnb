...
Le panel révèle deux géographies superposées et largement indépendantes : un palmarès du **prix médian** dominé par les métropoles anglo-saxonnes (Amsterdam, Édimbourg, Boston), et une géographie de la **structure de l'offre** où la Méditerranée affiche des taux de logements entiers proches de 100 % — marché d'investissement touristique pur. Le revenu hôte dépend autant du prix que de l'occupation : Amsterdam plafonne sous 8 000 €/an malgré ses 200 €/nuit, tandis que `r aa$t_top_rev` culmine à `r aa$v_top_rev_km` €/an grâce à un calendrier saturé.
...
kpi_e2("Panel d'étude · juin 2025", list(
...
Trois clivages structurent les écarts entre continents : un facteur prix moyen de 2 entre l'Europe du Sud (~80 €/nuit) et l'Amérique du Nord (~150 €), une part de logements entiers proche de 100 % en Méditerranée contre moins de 70 % en Asie, et un revenu annuel estimé qui suit le prix mais s'amplifie via l'occupation — l'Amérique du Nord dépasse les 7 000 €/an quand l'Asie reste sous 2 000 €.
...
<div class="figure-source">Source : Inside Airbnb, juin 2025 · Afrique masquée (n &lt; 2 villes) · Coloration z-score par ligne (rose = au-dessus de la moyenne mondiale, bleu = en-dessous).</div>
...
<div class="figure-source">Source : Inside Airbnb, juin 2025 · Médiane par groupe continental</div>
...
rt_table(p2_ville_df, cols = list(
  ...
  act_revenu_med     = rt_col_level(p2_ville_df, "act_revenu_med", "Revenu/an", unit = "€",
   ...
   footnote = "Inside Airbnb, juin 2025 · Tri par prix médian décroissant")
...
Le prix médian du logement entier oppose deux blocs continentaux : l'Europe du Sud-Est plafonne autour de 80 €/nuit quand l'Amérique du Nord dépasse 150 €. L'arc premium `r aa$t_top_prix`-Édimbourg-Boston franchit 200 € la nuit, tandis que les marchés accessibles — `r aa$t_bot_prix`, Thessalonique, Riga — restent sous 60 €. Paris (`r aa$v_paris_prix` €) se positionne au-dessus de la médiane européenne (`r aa$v_avg_eur_prix` €), sans atteindre les sommets anglo-saxons.
...
srpt02_struct <- kpi |> dplyr::select(city_fr, cd, country_code, str_entire_pct, str_cap_pers_med, str_cap_chbr_med)
rt_topbot_ville(
  srpt02_struct,
  vars = c("str_entire_pct", "str_cap_pers_med", "str_cap_chbr_med"),
  sort_col = "str_entire_pct",
  title = "Structure par ville — part de logements entiers et capacité",
  labels = c("% logements entiers", "Capacité (pers.)", "Chambres méd."),
  units = c("%", "pers", "n"),
  digits = c(1, 1, 1),
  footnote = "Pastille = sous-continent · barre % logements entiers = écart à la moyenne du pool (rose ▲ / bleu ▼) · footer = médiane Monde."
)
```
...
srpt02_rev <- kpi |> dplyr::select(city_fr, cd, country_code, act_revenu_med, act_reserv_taux, act_cal_ouvert_med)
rt_topbot_ville(
  srpt02_rev,
  vars = c("act_revenu_med", "act_reserv_taux", "act_cal_ouvert_med"),
  sort_col = "act_revenu_med",
  title = "Revenu et activité par ville — le revenu suit l'occupation",
  labels = c("Revenu estimé", "Taux d'occupation", "Calendrier ouvert"),
  units = c("€/an", "%", "j/an"),
  digits = c(0, 1, 0),
  footnote = "Pastille = sous-continent · barre Revenu estimé = écart à la moyenne du pool (rose ▲ / bleu ▼) · footer = médiane Monde."
)
```

:::

Le revenu d'un hôte ne suit pas le prix de la nuit mais l'occupation. `r aa$t_top_rev` génère `r aa$v_top_rev` €/an grâce à un calendrier saturé, quand Amsterdam — pourtant à `r aa$v_top_prix` €/nuit — plafonne sous 8 000 €/an. La différence se joue sur la disponibilité réelle du logement et le taux d'occupation effectif.
