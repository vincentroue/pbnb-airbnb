...
title: "ACP-HCPC — Pbnb 74 villes monde"
subtitle: "74 villes Airbnb · KPI agrégés Inside Airbnb juin 2025"
categories: ["Notebook"]
date: today
lang: fr
from: markdown+mark
...
if (!is.null(.p_ddict) && file.exists(.p_ddict)) {
  ...
  # &cld Le CSV ddict-validate n'a que short/long → on récupère le label MEDIUM
  # depuis le JSON ddict (source des labels) pour que le pivot affiche le medium.
  .json_dd <- "C:/Users/vince/hh/pq/PDS/pbnb-airbnb-log-jrr-jpy/reports/helpers/ddict-airbnb.json"
  if (!is.null(DDICT_DF) && "variable" %in% names(DDICT_DF) &&
      file.exists(.json_dd) && requireNamespace("jsonlite", quietly = TRUE)) {
    .jind <- jsonlite::fromJSON(.json_dd, simplifyVector = FALSE)$indicators
    .med <- vapply(DDICT_DF$variable, function(v) {
      m <- .jind[[v]]$medium
      if (is.null(m) || !nzchar(m)) NA_character_ else m
    }, character(1))
    DDICT_DF$medium <- ifelse(is.na(.med),
                              if ("short" %in% names(DDICT_DF)) DDICT_DF$short else NA_character_,
                              .med)
  }
...
if (nzchar(.ddict_path_safe) && file.exists(.ddict_path_safe) &&
  ...
  if (nrow(tr_res$transformations) > 0) {
    ...
    # &cld MAJ DDICT_DF (pivot rôles) : 1 ligne par variante transformée X_log,
    # copiée de la base X (medium/short/long suffixés " (log)", theme/unit conservés)
    # → le pivot affiche le label medium + le thème au lieu du nom brut + "?".
...
<!-- Variables dynamiques MÉTIER (dim_names/comments + cluster_names/comments).
     Chunk silencieux, rendu INVISIBLE ; édité dans _ccom-acp-vardyn.qmd. -->
...
saveRDS(acp_bilan_snapshot, "outputs/acp-bilan-snapshot.rds")
```
...
# &cld Garantir le label MEDIUM ici (cache:false, dernier) — robuste au cache des
# chunks amont. Le CSV n'a que short ; medium vient du JSON ddict. Gère les variantes
# transformées X_log : strip suffixe -> medium de la base -> re-suffixe " (log)".
if (!is.null(ddict_for_pivot) && "variable" %in% names(ddict_for_pivot)) {
  .jpath <- "C:/Users/vince/hh/pq/PDS/pbnb-airbnb-log-jrr-jpy/reports/helpers/ddict-airbnb.json"
  if (file.exists(.jpath) && requireNamespace("jsonlite", quietly = TRUE)) {
    .ji <- jsonlite::fromJSON(.jpath, simplifyVector = FALSE)$indicators
    .sfx_re <- "_(log|arcsin|yj|winsor|sqrt)$"
    ddict_for_pivot$medium <- vapply(seq_len(nrow(ddict_for_pivot)), function(i) {
      v <- ddict_for_pivot$variable[i]
      base <- sub(.sfx_re, "", v)
      m <- .ji[[base]]$medium
      if (is.null(m) || !nzchar(m))
        return(if ("short" %in% names(ddict_for_pivot)) ddict_for_pivot$short[i] else v)
      if (grepl(.sfx_re, v)) paste0(m, " (", sub(paste0(".*", .sfx_re), "\\1", v), ")") else m
    }, character(1))
  }
}

.label_col_auto <- if (!is.null(ddict_for_pivot) && "medium" %in% names(ddict_for_pivot)) "medium" else "short"
pivot_df <- build_config_pivot(CONFIG$grid_configs, ddict_for_pivot,
  theme_col = "theme", var_col = "variable", label_col = .label_col_auto,
