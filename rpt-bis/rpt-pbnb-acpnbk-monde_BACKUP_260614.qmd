...
<!-- Définit dim_names / cluster_names (chunk silencieux) pour cartes + biplot.
     Rendu INVISIBLE (aucune prose ici ; la lecture métier des axes est en §5).
     STUB requis : créer _ccom-acp-dim.qmd dans le projet (sinon render plante). -->
{{< include _ccom-acp-dim.qmd >}}

<!-- Nommage métier des clusters, AU-DESSUS des profils (intro). Jaune .cld-add. -->
::: {.cld-add}
{{< include _ccom-acp-cluster.md >}}
:::
...
# Profils des clusters (cartes 2-up, fond cluster léger rgba) = LA réponse. Body.
# cluster_names (noms métier digérés) vient du chunk de _ccom-acp-dim.qmd inclus plus haut.
render_cluster_cards(hcpc, indiv_df, ddict = DDICT, actives = rownames(pca$var$coord),
                     cluster_names = cluster_names, top_n_para = 6)
```
...
.yl <- range(c(.icoord[, 2], .icoord[, 3])) * 1.15
.xl <- range(.icoord[, 1]) * 1.15
...
**Ce que disent les axes** — lecture automatique des pôles (quelles variables structurent chaque dimension) :

```{r}
#| label: bilan-dim-themes
#| echo: false
#| results: asis

# Commentaire AUTO par dimension (poles + / -) : complete la lecture metier ci-dessous.
render_acp_dim_themes(pca,
  n_dims = min(3, max(aa_n_kaiser, aa_n_horn, na.rm = TRUE)),
  ddict = DDICT, n_top = 4)
```

<!-- Lecture metier d'ensemble : _ccom-acp-bilan.md. Jaune .cld-add. -->
::: {.cld-add}
...
# Lecture AUTO des pôles, SOUS le métier ci-dessus : top contrib Dim1/Dim2, noms de
# variables TRADUITS via DDICT (sinon code R). ==marker== pour Pandoc +mark.
...
# Lecture AUTO du pôle Dim3, SOUS le métier ci-dessus. Noms TRADUITS via DDICT.
...
*Lecture automatique (pôles + / − par dimension, top contributions) :*
...
# Cartes 2 par ligne (.grid/.g-col-6) fond cluster leger : phrase dynamique
# (sur/sous-representation) + parangons + atypiques. Labels DDICT short.
render_cluster_cards(hcpc, indiv_df, ddict = DDICT, actives = rownames(pca$var$coord),
                     cluster_names = cluster_names, top_n_para = 6)
```
