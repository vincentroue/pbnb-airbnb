...
# (ex decathlon 100m/400m/1500m) qui sinon deviennent X100m… et ne matchent plus
...
if (nzchar(.ddict_path_safe) && file.exists(.ddict_path_safe) &&
  ...
  # Fix pbnb (260614) : le transform par-variable est deja applique (swap var_log
  # ci-dessus). On NEUTRALISE le transform GLOBAL herite de load_grid (calcul
  # "majoritaire" : 1 active en log => "log" global) sinon prepare_pca_hcpc
  # re-loggue TOUTES les actives. Avec ce fix : seules les vars taggees _transform
  # sont transformees, le reste reste brut (profils interpretables).
  CONFIG$transform <- "none"
  if (!is.null(CONFIG$grid_configs))
    CONFIG$grid_configs <- lapply(CONFIG$grid_configs, function(cfg) {
      cfg$transform <- "none"; cfg })
}
# &e
...
<!-- Variables dynamiques MÉTIER (dim_names/comments + cluster_names/comments).
     Chunk silencieux, rendu INVISIBLE. STUB requis : créer _ccom-acp-vardyn.qmd
     dans le projet (sinon render plante). -->
...
```{r}
#| label: plt-bilan-clusters-dual
#| echo: false
#| out-width: "100%"
#| fig-width: 13
#| fig-height: 5.5

# Cartes clusters CÔTE À CÔTE (Axes 1-2 et 1-3) reprises dans le bilan, en BODY
# (out-width 100% remplit la colonne prose ; pas full page / pas column-screen).
.icoord <- pca$ind$coord
.cen <- apply(.icoord[, 1:3], 2, function(v) tapply(v, hcpc$data.clust$clust, mean))
# Zoom : englobe les centroïdes + ~96 % des individus (P2-P98), rogne les extrêmes -> moins de blanc.
.qx <- quantile(.icoord[, 1], c(.02, .98)); .qy <- quantile(c(.icoord[, 2], .icoord[, 3]), c(.02, .98))
.xl <- range(c(.qx, .cen[, 1])) * 1.06
.yl <- range(c(.qy, .cen[, 2], .cen[, 3])) * 1.06
(plot_ind_cluster(pca, hcpc, axes = c(1, 2), label_parangons = TRUE, dim_names = dim_names, cluster_names = cluster_names) + coord_cartesian(xlim = .xl, ylim = .yl)) +
(plot_ind_cluster(pca, hcpc, axes = c(1, 3), label_parangons = TRUE, dim_names = dim_names, cluster_names = cluster_names) + coord_cartesian(xlim = .xl, ylim = .yl)) +
...
# MÉTIER (dim_comments des axes 1 & 2) en tête, puis lecture AUTO des pôles en dessous.
...
.wrap_mark <- function(v) if (length(v) == 0) "(aucune)" else paste(sprintf("==%s==", .lblv(v)), collapse = ", ")

cat("*Lecture automatique des pôles (variables aux plus fortes contributions) :*\n\n")
cat(sprintf("**Axe 1 (%.1f %% de variance)** oppose deux pôles : à droite %s ; à gauche %s. Gradient continu — les individus à l'extrémité droite ont des valeurs élevées sur les premières, faibles sur les secondes.\n\n",
  aa_v_dim1, .wrap_mark(pos1), .wrap_mark(neg1)))
cat(sprintf("**Axe 2 (%.1f %% de variance)** est orthogonal à Dim1. Pôle haut : %s ; pôle bas : %s. À niveau Dim1 équivalent, cet axe sépare des sous-profils.\n\n",
...
# MÉTIER (dim_comments axe 3) en tête, puis lecture AUTO du pôle en dessous.
...
cat("*Lecture automatique du pôle (variables aux plus fortes contributions) :*\n\n")
...
# MÉTIER (dim_comments des 3 axes) en tête, puis lecture AUTO en dessous.
for (.i in seq_along(dim_names))
  cat(sprintf("**Axe %d — %s.** %s\n\n", .i, dim_names[.i], dim_comments[.i]))
cat("*Lecture automatique (pôles + / − par dimension, top contributions) :*\n\n")
...
```{r}
#| label: intro-clusters-axes
#| echo: false
#| results: asis

# Rappel métier des axes (dim_comments) AVANT le biplot des clusters.
cat("**Avant de lire les clusters — ce que disent les axes :**\n\n")
for (.i in seq_along(dim_names))
...
shared_xlim <- range(c(.qx, .cen[, 1])) * 1.06
shared_ylim <- range(c(.qy, .cen[, 2], .cen[, 3])) * 1.06
...
p13 <- plot_ind_cluster(pca, hcpc, axes = c(1, 3), label_parangons = TRUE, dim_names = dim_names, cluster_names = cluster_names) +
  coord_cartesian(xlim = shared_xlim, ylim = shared_ylim)
# Titre COMMUN aux 2 panneaux (plot_annotation) ; titres de panneaux = noms de dimensions.
p12 + p13 +
  plot_layout(ncol = 2, guides = "collect") +
  plot_annotation(title = sprintf("Typologie des %d clusters", aa_n_bestk),
...
typo_individus_rt(hcpc, pca, vars = CONFIG$actives,
  label_col = CONFIG$label_col,
  cap_z = 2.0, band = 0.3, page_size = 12, height = "560px", max_rows = 2000)
```
:::
